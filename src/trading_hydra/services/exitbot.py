...
class ExitBot:
    ...
    def run(self, equity: float, day_start_equity: float) -> ExitBotResult:
        ...
        # ------------------------------------------------------------------
        # Step 7.5: Periodic staged order reconciliation (~every 5 min)
        # Cleans up ghost orders that no longer exist on Alpaca
        # ------------------------------------------------------------------
        if self._run_counter % 20 == 0:
            try:
                reconcile_result = self.reconcile_staged_orders()
                if reconcile_result.get("cleaned_up", 0) > 0:
                    self._logger.log("exitbot_staged_orders_reconciled", reconcile_result)
            except Exception as e:
                self._logger.error(f"Staged order reconciliation failed (fail-open): {e}")
            ...
            if is_new_position:
                ...
                if ts_state is not None:
                    ...
                    if updated_state is None:
                        self._logger.error(f"Trailing stop update_state returned None for {position_info.symbol} — falling back to ts_state (fail-open)")
                        updated_state = ts_state
                    ...
                    if updated_state.armed:
                        _tightened = self._trailing_stop_mgr.apply_profit_tier_tightening(
                            updated_state, position_info.current_price
                        )
                        if _tightened is not None:
                            updated_state = _tightened
                    ...
                    if tighten_stops and updated_state.armed:
                        _tightened = self._trailing_stop_mgr.apply_tightening(
                            bot_id=position_info.bot_id,
                            position_id=position_info.position_id,
                            symbol=position_info.symbol,
                            asset_class=position_info.asset_class,
    ...
    def _close_with_spread_protection(self, position_info: 'PositionInfo', reason: str) -> bool:
        ...
        try:
            # Cancel staged exit orders + any open broker orders BEFORE closing
            # to prevent Alpaca wash-trade rejection (error 40310000 / held_for_orders)
            try:
                self.cancel_staged_orders_for_symbol(position_info.symbol, "exit_pre_cancel")
            except Exception as _pre_cancel_err:
                self._logger.error(f"Pre-cancel staged orders failed for {position_info.symbol}: {_pre_cancel_err}")
            try:
                _open_orders = self._alpaca.get_open_orders(position_info.symbol)
                for _o in _open_orders:
                    _oid = _o.get("id") or _o.get("order_id", "")
                    if _oid:
                        self._alpaca.cancel_order(_oid)
            except Exception as _broker_cancel_err:
                self._logger.error(f"Pre-cancel broker orders failed for {position_info.symbol}: {_broker_cancel_err}")
    ...
    def _add_exit_record(self, position: PositionInfo, reason: str) -> None:
        ...
        try:
            ...
            # Update daily trade metrics so orchestrator end-of-day reporting has real data
            try:
                _count = int(get_state("daily.trades_count") or 0) + 1
                set_state("daily.trades_count", _count)
                if pnl > 0:
                    set_state("daily.wins_count", int(get_state("daily.wins_count") or 0) + 1)
                    _wins = list(get_state("daily.win_amounts") or [])
                    _wins.append(round(pnl, 2))
                    set_state("daily.win_amounts", _wins)
                else:
                    _losses = list(get_state("daily.loss_amounts") or [])
                    _losses.append(round(abs(pnl), 2))
                    set_state("daily.loss_amounts", _losses)
            except Exception as _dm_err:
                self._logger.error(f"Daily metrics update failed (fail-open): {_dm_err}")
